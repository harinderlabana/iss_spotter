//contain most of the logice for fetching the data from api endpoint
const request = require('request');

/**
 * Makes a single API request to retrieve the user's IP address.
 * Input:
 * - A callback (to pass back an err or the IP string)
...
const fetchISSFlyOverTimes = function(coords, callback) {
  request(
    ...
    (error, response, body) => {
      ...
      if (response.statusCode !== 200) {
        const msg = `Status Code ${respsonse.statusCode} when fetching ISS pass times. Response: ${body}`;
        callback(Error(msg), null);
        return;
      }
